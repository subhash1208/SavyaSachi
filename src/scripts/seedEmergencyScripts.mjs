/**
 * One-time seed script: populate vaidyavaani-emergency-scripts DynamoDB table.
 * Run from CloudShell: node seedEmergencyScripts.mjs
 *
 * Table schema:
 *   PK: condition_id (String)
 *   SK: patient_category (String)
 *
 * For hackathon: all conditions seeded with patient_category = "adult".
 * child_fever also seeded with patient_category = "pediatric".
 * The service falls back to "adult" if requested category not found.
 */
...
const SCRIPTS = [
...

  {
    ...
    doNotActions: [
      ...
      { hindi: 'Jhatkay ke waqt bachche ke munh mein kuch bhi mat daalein.', english: 'Do NOT put anything in the mouth during a convulsion.' },
  ...
  {
    condition_id: 'breathing_difficulty',
    patient_category: 'adult',
    icd10Code: 'J45.9',
    dispatchType: '108',
    severity: 'CRITICAL',
...
// Stub script generator for remaining 11 conditions
const STUB_CONDITIONS = [
  { condition_id: 'stroke',               icd10Code: 'I64'   },
  { condition_id: 'severe_bleeding',      icd10Code: 'R58'   },
  { condition_id: 'choking',              icd10Code: 'T17.9' },
  { condition_id: 'burns',               icd10Code: 'T30.0' },
  { condition_id: 'poisoning',           icd10Code: 'T65.9' },
  { condition_id: 'anaphylaxis',         icd10Code: 'T78.2' },
  { condition_id: 'seizure',             icd10Code: 'R56.9' },
  { condition_id: 'pregnancy_emergency', icd10Code: 'O14.9' },
  { condition_id: 'drowning',            icd10Code: 'T75.1' },
  { condition_id: 'unconsciousness',     icd10Code: 'R40.2' },
  { condition_id: 'infant_not_breathing',icd10Code: 'P28.4' },
  { condition_id: 'heatstroke',          icd10Code: 'T67.0' },
];
...
for (const c of STUB_CONDITIONS) {
  SCRIPTS.push({
...
async function seed() {
  ...
  for (const script of SCRIPTS) {
    await client.send(new PutItemCommand({
      TableName: TABLE,
      Item: marshall(script, { removeUndefinedValues: true }),
    }));
    count++;
    console.log(`[${count}/${SCRIPTS.length}] Seeded: ${script.condition_id} / ${script.patient_category}`);
  }
  console.log(`\nDone. ${count} scripts written to ${TABLE}`);
}
